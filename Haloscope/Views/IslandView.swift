...
struct IslandView: View {
    ...
    private var details: some View { IslandNativeScrollView { VStack(alignment:.leading,spacing:10) {
        ...
        section("账户额度",icon:"gauge.with.dots.needle.50percent") {
            ...
            HStack {
                ...
                Spacer()
                Text(model.availableResetCredits.map(String.init) ?? "—").monospacedDigit().fontWeight(.semibold)
            }.font(.system(size:11)).foregroundStyle(.secondary)
        }
        section("当前任务",icon:model.hasRecentThreadActivity ? "clock.arrow.circlepath":"moon") { HStack(spacing:10) { Circle().fill(statusColor.opacity(0.18)).frame(width:30,height:30).overlay(Image(systemName:model.hasRecentThreadActivity ? "clock.arrow.circlepath":"moon").font(.system(size:12,weight:.bold)).foregroundStyle(statusColor)); VStack(alignment:.leading,spacing:2) { Text(model.hasRecentThreadActivity ? "最近有线程活动 · 推断":"无活动").font(.system(size:13,weight:.medium)); Text(model.monitoredThread?.preview ?? "没有最近线程").font(.system(size:11)).foregroundStyle(.secondary).lineLimit(1); if let cwd=model.monitoredThread?.cwd { Text(cwd).font(.system(size:9).monospaced()).foregroundStyle(.tertiary).lineLimit(1) } }; Spacer() } }
        section("最近对话",icon:"text.bubble") { if model.threads.isEmpty { empty("暂无线程数据") } else { ForEach(model.threads.prefix(3)) { thread in threadRow(thread) } } }
        section("Token 使用",icon:"chart.bar.xaxis") { if let usage=model.usageSummary { HStack(spacing:8) { metric("最近统计日",usage.buckets.last?.tokens ?? 0,subtitle:usage.buckets.last?.startDate.formatted(.dateTime.month().day())); metric("7 天",usage.sum(days:7)); metric("30 天",usage.sum(days:30)) } } else { empty("账户用量数据不可用") } }
        if let usage=model.usageSummary { section("Codex 统计",icon:"chart.line.uptrend.xyaxis") { codexStatistics(usage) } }
    ...
    private func metric(_ label:String,_ value:Int,subtitle:String?=nil)->some View { VStack(alignment:.leading,spacing:3) { HStack(spacing:4) { Text(label).font(.system(size:9,weight:.medium)); if let subtitle { Text(subtitle).font(.system(size:8)) } }.foregroundStyle(.tertiary); Text(value.formatted(.number.notation(.compactName))).font(.system(size:15,weight:.semibold).monospacedDigit()) }.padding(9).frame(maxWidth:.infinity,alignment:.leading).background(Color.black.opacity(0.11),in:RoundedRectangle(cornerRadius:9,style:.continuous)).overlay(RoundedRectangle(cornerRadius:9,style:.continuous).stroke(Color.white.opacity(0.07),lineWidth:0.5)) }
    private func codexStatistics(_ usage:UsageSummary)->some View { HStack(spacing:0) { statistic("累计 Token",usage.lifetimeTokens.map(compact) ?? "—"); divider; statistic("峰值 Token",usage.peakDailyTokens.map(compact) ?? "—"); divider; statistic("最长任务",usage.longestRunningTurnSec.map(duration) ?? "—"); divider; statistic("当前连续",usage.currentStreakDays.map{"\($0) 天"} ?? "—"); divider; statistic("最长连续",usage.longestStreakDays.map{"\($0) 天"} ?? "—") } }
    private func statistic(_ label:String,_ value:String)->some View { VStack(spacing:3) { Text(value).font(.system(size:13,weight:.semibold).monospacedDigit()).foregroundStyle(accent); Text(label).font(.system(size:8,weight:.medium)).foregroundStyle(.secondary).lineLimit(1) }.frame(maxWidth:.infinity) }
    ...
    private func duration(_ seconds:Int)->String { let hours=seconds/3600, minutes=(seconds%3600)/60; return hours > 0 ? "\(hours)时\(minutes)分":"\(minutes) 分" }
